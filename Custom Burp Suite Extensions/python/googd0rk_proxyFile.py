#python script.py example.com -p proxies.txt
#python script.py example.com --proxy_file proxies.txt
# cat proxies.txt
#
#http://1.1.1.1
##http://1.1.1.1:5555
...
def googd0rk(query, user_agent, proxy):
...
def dorkthosethreads(query, user_agent, output_file, proxy):
  # Perform the dorking
  result = googd0rk(query, user_agent, proxy)
...
def main():
  ...
  parser.add_argument("-p", "--proxy_file", default="proxies.txt", help="the file containing the proxy URLs")
  ...
  # Read the proxy URLs from a file
  with open(args.proxy_file, "r") as proxies_file:
  ...
  for query in queries:
    ...
    t = threading.Thread(target=dorkthosethreads, args=(query, user_agent, args.output_file, proxy))
